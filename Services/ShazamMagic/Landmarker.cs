...
namespace CSharpWpfShazam.Services.ShazamMagic
{
	...
	internal class Landmarker {
		...
		// In DN4.8, MathF not available, Math.Log() works with double (not float)
		private static readonly float LogMinMagnitudeSquared = (float)Math.Log((float)MinMagnitudeSquared);
		...
		private float GetLogMagnitude(int stripe, int bin) {
			return 18 * 1024 * (1 - (float)(Math.Log((float)_analysis.GetMagnitudeSquared(stripe, bin)) / LogMinMagnitudeSquared));
		}
